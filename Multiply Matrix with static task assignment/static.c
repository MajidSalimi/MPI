...
//---------------------------------------------------------------------
    ...
    int main(int argc, char **argv){
     double t1_p,t2_p,t1_s,t2_s;
      int i,u, myid, procs,j,z,trow,temp1,k,t,s,h,jj;
      ...
      if (myid == 0)
	   {
			...
			while(j<m)
			{
				...
				for(s=0;s<procs && i< m ;s++)
				{
					...
					{						
						...
						temp1= i;						
						MPI_Send(&temp1, 1, MPI_INT, s , 1, MPI_COMM_WORLD);
						i+=k;
					}
					else 
					{ 
						 if(i == m-R)
							temp1=R;
							  ...
							  temp1=k;						
						 for (z=0;z<temp1;z++)//do computation
			               for(h=0;h<p;h++)
			                {
				             temp[z*p+h]=0;
			                 for(jj=0;jj<n;jj++)
			                  temp[z*p+h]+=(arr1[(z+i)*n+jj]*arr2[jj*p+h]);
			                }//end for i=0
		            	  //end for z =0
		            	 printf("\nFrom processor 0 I received:\n");  
					     printArray(temp,k,p);
		            	 for(z=0;z<temp1;z++)
							for(u=0;u<p;u++)
								result[(i+z)*p+u]=temp[z*p+u];//copy temp results that receives from each processor to result
			...
			for(s=1;s<procs;s++)
			{
			   MPI_Send(&i, 1, MPI_INT, s, 0, MPI_COMM_WORLD);//tag=0 Means that work finished	
			}  				          			  
